const char *dgemm_desc = "Opt5: No register accum (2-level blocking, pragmas, restrict)";
...
/* Micro-kernel without packing or register accumulation */
static inline void do_block_micro(int lda, int M, int N, int K,
                                  double *restrict A, double *restrict B,
                                  double *restrict C) {
  /* Compute directly to C (no local accumulator) */
  for (int j = 0; j < N; ++j) {
    for (int k = 0; k < K; ++k) {
