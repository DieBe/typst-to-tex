const char *dgemm_desc = "Opt6: No packing (2-level blocking, register accum, pragmas, restrict)";
...
/* Micro-kernel without packing, but with register accumulation */
static inline void do_block_micro(int lda, int M, int N, int K,
  ...
  /* Compute directly on original data (strided access) */
  for (int j = 0; j < N; ++j) {
    for (int k = 0; k < K; ++k) {
      double b_kj = B[k + j * lda];
      #pragma GCC ivdep
      for (int i = 0; i < M; ++i) {
        cij[i + j * M] += A[i + k * lda] * b_kj;
      }
    }
  }
